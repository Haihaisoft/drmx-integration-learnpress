...
$order_query = mysqli_query($dbcon,"SELECT post_id FROM ".$table_prefix."postmeta WHERE meta_key= '_user_id' AND meta_value = '" .$userid. "'");

while($order_result = mysqli_fetch_array($order_query, MYSQLI_ASSOC)){
	$PostID = $order_result['post_id'];

	// Get order_item_id based on post_id
	$order_item_id_query = mysqli_query($dbcon,"SELECT order_item_id FROM ".$table_prefix."learnpress_order_items WHERE order_id = '" .$PostID. "'");
	...
	while($order_item_id_result = mysqli_fetch_array($order_item_id_query, MYSQLI_ASSOC)){
		...
		// Get product ID based on Order ID
		$course_query = mysqli_query($dbcon,"SELECT meta_value FROM ".$table_prefix."learnpress_order_itemmeta WHERE meta_key = '_course_id' AND learnpress_order_item_id = '" .$order_item_id. "'");
		$course_result = mysqli_fetch_array($course_query);
		$course_id = $course_result['meta_value'];
		
		/***** Verify whether the user has purchased the course. ****/
		foreach ($PIDs as $ProductID) {
			if($course_id == $ProductID){

				$status_query = mysqli_query($dbcon,"SELECT post_date, post_status FROM ".$table_prefix."posts WHERE ID = ".$PostID);
				$status_result = mysqli_fetch_array($status_query, MYSQLI_ASSOC);
				$order_status = $status_result['post_status']; //get order status

				// Verify the order status is completed
				if($order_status == "lp-completed"){
					...
					$duration_result = mysqli_fetch_array($duration_query, MYSQLI_ASSOC);
					$lp_duration = $duration_result['meta_value']; //get order status

					if(strpos($lp_duration,"week") > 0){
						$arr = explode(" ",$lp_duration);
						$lp_duration = $arr[0]*7;
					}else{
						$lp_duration = explode(" ",$lp_duration)[0];
					}
...
if($flag == 0) $info = "You have not purchased this course yet, can not open this file! <br> Please buy a course first! <br> If you have questions, Please contact us!";
